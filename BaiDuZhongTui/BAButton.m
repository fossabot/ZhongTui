...
{
    BAButton *button = [BAButton buttonWithType:UIButtonTypeCustom];
    button.frame = frame;
    if (title)
    {
...
- (void)alignmentTop
{
    ...
    self.titleLabel.frame = CGRectMake((self.center.x - frame.size.width) * 0.5, BA_btnHeight * 0.5 + BA_labelHeight * BA_ButtonTopRadio + self.padding, BA_btnWidth, BA_labelHeight);
...
- (void)layoutSubviews
{
    ...
    [self setupButtonPositionStyle];
    if (_hadWhiteBackground) {
        UIView *whiteView = [[UIView alloc] init];
        whiteView.center = self.imageView.center;
        CGRect bounds = whiteView.bounds;
        bounds.size = CGSizeMake(50, 50);
        whiteView.bounds = bounds;
